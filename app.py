...
st.sidebar.markdown(f"🟢 **AHTN 2022 Database**: Loaded ({len(ahtn_df)} rows)")
...
if nav_selection == "Home / Executive Dashboard":
    ...
    st.markdown("Independent institutional decision-support platform evaluating preferential tariffs, dual-API trade flows, and origin compliance.")
    st.markdown("---")
    
    c1, c2, c3, c4 = st.columns(4)
    c1.metric("Nomenclature", "AHTN 2022")
    c2.metric("Supported FTAs", "10+ Agreements")
    c3.metric("Live APIs", "Comtrade & WITS")
    ...
    st.title("Economic Zones & BARMM")
    st.markdown("Explore PEZA, freeports, and BEZA investment incentives under the CREATE MORE Act.")

elif nav_selection == "Data Sources & Provenance":
    st.title("Data Sources & Provenance")
    st.markdown("All datasets are sourced directly from UN Comtrade API, World Bank WITS API, ASEAN Secretariat, and Philippine Tariff Commission.")
